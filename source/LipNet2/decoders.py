...
def _decode(y_pred, input_length, greedy=True, beam_width=100, top_paths=1):
    ...
    # MAHSA: changes in session because of eval!
    temp_path = np.array(decoded[0])
    temp_logprobs=np.array(decoded[1])
    # print(f"decoded[0] size: {temp_path[0,:,:].shape} and content {temp_path[0,:,:]}")
    # print(f"decoded[1] size: {temp_logprobs[0,:].shape} and content {temp_logprobs[0,:]}")

    # paths = [path.eval(session=K.get_session()) for path in decoded[0]]
    # logprobs  = np.array(decoded[1]).eval(session=K.get_session())
    paths = temp_path
