...
def ctc_lambda_func(args):
    ...
    y_pred = y_pred[:,:,:]
    # print(f"shape of y_pred after: {y_pred.shape}")
    ctc_loss = K.ctc_batch_cost(labels, y_pred, input_length, label_length)
    # print(f"ctc loss shape {ctc_loss.shape}")
    return ctc_loss

def CTC(name, args):
    lambda_lay = Lambda(ctc_lambda_func, output_shape=(1,), name=name)(args)
    # print(f"shape of Lambda layer {lambda_lay.shape}")
...
class LipNet(object):
    ...
    def predict(self, input_batch): ### MAHSA: predict and test_function has been changed

        # return self.test_function([input_batch, 0])[0]  # the first 0 indicates test

        # self.partial_model = Model(inputs=self.model.inputs, outputs=self.model.layers[23].outputs)
        # output_train = partial_model(x, training=True)  # runs the model in training mode
        output_test = self.partial_model(input_batch, training=False)  # runs the model in test mode
        # fn = K.function(self.input_data, self.y_pred)
        # output_test = fn(input_batch)
    ...
    @property ### MAHSA: This function has been combined with the predict.
