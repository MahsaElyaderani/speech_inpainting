from LipNet2.helpers import text_to_labels
...
def get_list_safe(l, index, size):
    ...
    #    print(f"index is {index}, size is {size} and len(ret) is {len(ret)}")
    #    ret += l[0:size - len(ret)]
    #    print(f"added ret is {ret}")
    return ret


class BasicGenerator(tensorflow.keras.callbacks.Callback):
    def __init__(self, dataset_path, minibatch_size, img_c, img_w, img_h, frames_n, absolute_max_string_len=30,
        ...
        self.vtype = kwargs.get('vtype', 'mouth') #, 'face') #
    ...
    def build_dataset(self):
        print("cache path", self.get_cache_path())
    ...
    def get_batch(self, index, size, train):
        ...
        for path in X_data_path:
            ### MAHSA: following line should use from_video(path), if I want to use video files.
            video = Video().from_frames(path)#from_video(path)#
            # print("align", path.split('/')[-1])
            ### MAHSA: I may need to change following line in split func.
            align = self.get_align(path.split('/')[-1])# self.get_align(path.split('/')[-1][:-4])
