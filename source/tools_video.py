...
def adjust_landmarks(landmarks, anchor_landmark=33):
    ...
    deleted_ids = list(range(anchor_landmark * 2, landmarks.size, 136)) + list(
        range(anchor_landmark * 2 + 1, landmarks.size, 136))
...
def extract_face_landmarks(video_filename, predictor_params, refresh_size=8):
    ...
    # detector = dlib.cnn_face_detection_model_v1('/Users/kadkhodm/PycharmProjects/speech_inpainting/source/mmod_human_face_detector.dat') #cnn_face_detector
    ...
    while cap.isOpened():
        ...
        if ret == True:
            ...
            if rect:
                ...
                # motion.append(get_motion_vector(face_utils.shape_to_np(shape), delta=1))
    ...
    return np.array(landmarks), np.array(face_rects), flag_face_found


def show_face_landmarks(video_filename, predictor_params, full_draw=False, bb_draw=False, frame_draw=True, fps=25.0,
                        out_video_filename="", refresh_size=8):
    """
...
def save_face_landmarks(dataset_path, speakers_list, video_dir, dest_dir, predictor_params, file_ext='mpg',
                        refresh_size=8):
...
def inc_fps(frames, target_len):
    ...
    #print("x: ", x)
    #print("y: ", y)
    interp_spline = RectBivariateSpline(y, x, frames)
    interp_vid = interp_spline(y_inc, x)
    # interp_vid = interp2d(x, y, frames, kind='linear')(x, y_inc)

    return interp_vid


def sync_audio_visual_features(len_mask, video_features, tot_frames=None, min_frames=None, pad='start'):
    """
    ...
    video_features = inc_fps(video_features, len_mask)

    # Check dimensions
    if len_mask == len(video_features):
        return video_features
    else:
        return None


def lip_edges(video_frames):
    blur_frames = np.array([cv2.GaussianBlur(frame, (5, 5), 0) for frame in video_frames])
    blur_frames = np.expand_dims(blur_frames, axis=-1)
    edge_frames = np.array([cv2.Canny(np.uint8(img), 0, 5) for img in blur_frames])
    # edge_frames = np.expand_dims(edge_frames, axis=-1)
    # features[1:] = edge_frames[1:] - edge_frames[:-1]
    # video_features = self.inc_fps(features.reshape((75, -1)), len(mask2d))
    # video_features = np.reshape(video_features, (149, 50, 100, 1))
    edge_frames = edge_frames.astype(np.float16) / 255
    return edge_frames
